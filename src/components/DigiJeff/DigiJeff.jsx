...
import "./DigiJeff.css";
import videoSrc from "./../../assets/im-in.mp4";
...
const DigiJeff = ({
    ...
    entryMessage = "ENTER ENTRY MESSAGE",
    ...
    const digiJeffRef = useRef(null);
    const messageRef = useRef(null);
    const entryMessageRef = useRef(null);
    const speechBubbleRef = useRef(null); // Ref for the speech bubble

    useEffect(() => {
        gsap.set([digiJeffRef.current, entryMessageRef.current], { autoAlpha: 0, scale: 0.5, x: -100 });
        gsap.set(messageRef.current, { autoAlpha: 0, scale: 0.5 });

        setTimeout(() => {
            gsap.to([digiJeffRef.current, entryMessageRef.current], {
                autoAlpha: 1,
                scale: 1,
                x: 0,
                duration: 1.2,
